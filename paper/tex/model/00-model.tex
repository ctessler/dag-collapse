...
predecessor nodes. 
A path between two nodes $u, v \in V$ is represented by $u \prec^{x} v$, where $x$ denotes the length of the path. For example, $u \prec^{3} v$ denotes that there exists a path from $u$ to $v$ of length of 3. Similarly, $u \prec^{*} v$ denotes that there exists a path from $u$ to $v$ of some arbitrary length. 
