...
predecessor nodes. 
\revise{Dependency between two nodes $u$, $v$ is represented by $u \prec^{*} v$, where $\prec^{*}$ represents the precedence with an arbitrary length, i.e., $u$ precedes $v$ in the DAG by an arbitrary length. Similarly,  $u \prec^{3} v$ denotes that $u$ precedes $v$ by a length of 3.}
